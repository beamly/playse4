...
final class AtomicFuture[T](initialValue: Future[T]) extends AtomicReference[Future[T]](initialValue) {
  @tailrec
  def send(f: T => T)(implicit executor: ExecutionContext): Future[Unit] = {
    val current = this.get()
    val promise = Promise[T]()
    if (this compareAndSet(current, promise.future)) {
  ...
  @tailrec
  def flatSend(f: T => Future[T])(implicit executor: ExecutionContext): Future[Unit] = {
    val current = this.get()
    val promise = Promise[T]()
    if (this compareAndSet(current, promise.future)) {
